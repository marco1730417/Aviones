  ...
  //Cabecera de página
    function Header()
    {
       ...
       $this->Rect(1,31,213,1,'DF');
        ...
        $this->Cell(30);
        //Título
        $this->Cell(170,4,'REPORTE DE EMPLEADOS ',0,0,'C');
        $this->Ln(2);
      ...
      //  $this->MultiCell(170,5,'algun otro mensaje:');
        ...
        $this->Text(22,36,'Nombre');
        $this->Text(40,36,'Direccion');
        $this->Text(80,36,'Teléfono');
...
$conexion = pg_connect($cadconex);
  $cadbusca = "SELECT  nombre_empleado,direccion_empleado,telefono_empleado
         from empleado  WHERE nombre_empleado='".$empleado."'";
    ...
    while($row = pg_fetch_array($result))
        {
          ...
          $apellidos = $row["direccion_empleado"];
          $gerencia = $row["telefono_empleado"];
          $gerenci = "asegurado";
          $pdf->Text(22,$pdf->GetY(),$name);
          $pdf->Text(40,$pdf->GetY(),$apellidos);
          $pdf->Text(80,$pdf->GetY(),$gerencia);
          $pdf->Text(120,$pdf->GetY(),$gerenci);
          $pdf->cell(0,5.5,'',0,1);
        $j=$j+1;
        }
