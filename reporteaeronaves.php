  ...
  //Cabecera de página
    function Header()
    {
      ...
      //  $this->MultiCell(170,5,'algun otro mensaje:');
        ...
        $this->Text(150,36,'Dias');
        $this->Text(180,36,'Valor Dia');
...
$conexion = pg_connect($cadconex);
    ...
    $cadbusca = "select matricula,fecha_inicio ,fecha_fin,nombre_estado,dias,valor_dia,
        (select (sum(dias))from estado )as total_dias,
        (select (sum(valor_total))from estado)as valor_total
         from estado,aeronave where
         estado.id_aeronave=aeronave.id_aeronave
         and aeronave.id_poliza=".$poliza.""  ;
  ...
  if($nr>0){
    ...
    $pdf->AddPage();
         ...
         while($row = pg_fetch_array($result))
        {
          ...
          $pdf->Text(150,$pdf->GetY(),$dias);
          $pdf->Text(180,$pdf->GetY(),$valordia);
...
?>
<script>
alert('NO existen datos correspondientes a la poliza');
